...
| 可选浏览器后端 | Playwright、CloakBrowser |
...
| `--headless` | 关闭 | 使用无头模式运行。默认显示浏览器窗口，便于手动处理 CAPTCHA。 |
| `--no-headless` | 关闭 | 兼容旧参数；默认已经是有头模式。 |
| `--user-data-dir` | `profiles/default` | 指定持久化 Chrome 用户目录。 |
| `--browser-backend` | `playwright` | 后端，可选 `playwright`、`cloakbrowser`。 |
...
```bash
gemini-search --browser-backend playwright --channel chrome --port 8080
gemini-search --browser-backend cloakbrowser --port 8080
gemini-search --browser-backend playwright --headless --user-data-dir ./profile
```
...
| `HEADLESS` | `0` | 是否无头运行。设为 `1` 时不显示浏览器窗口。Docker 中默认显式设为 `1`。 |
| `GEMINI_SEARCH_USER_DATA_DIR` | `profiles/default` | 持久化 Chrome 用户目录。Docker 中默认显式设为 `/data/chrome-profile`。 |
...
| `GEMINI_SEARCH_BROWSER_BACKEND` | `playwright` | 后端：`playwright` 或 `cloakbrowser`。 |
| `GEMINI_SEARCH_GOOGLE_BASE_URL` | `https://www.google.co.uk` | Google 区域域名。 |
| `GEMINI_SEARCH_WARMUP_QUERY` | `latest UK technology news and weather in London this week` | 启动 warmup 时输入的自然搜索语。 |
| `GEMINI_SEARCH_PROXY_SERVER` | 无 | Chrome 代理地址。 |
| `GEMINI_SEARCH_CHROME_EXTRA_ARGS` | 无 | 追加 Chrome 启动参数，Docker 中常用 `--no-sandbox --disable-dev-shm-usage --enable-gpu --enable-webgl`。 |
...
```bash
gemini-search --user-data-dir "$HOME/.local/share/gemini-search-mcp/chrome-profile"
```
...
| `GEMINI_SEARCH_BROWSER_BACKEND` | 否 | 后端，默认 `playwright`。可选 `playwright`、`cloakbrowser`。 |
| `GEMINI_SEARCH_GOOGLE_BASE_URL` | 否 | Google 区域域名，默认 `https://www.google.co.uk`。 |
...
项目支持使用 PyInstaller `onedir` 模式构建 Windows 桌面运行目录。用户可以将源码文件夹复制到桌面，然后运行 PowerShell 构建脚本生成 exe。
...
- 检查 Python 是否存在。
- 如果没有 Python，尝试通过 `winget` 安装 Python 3.12。
- 在当前源码目录创建 `.venv-windows-build`。
- 安装项目依赖、Playwright 和 PyInstaller。
- 将 Playwright Chromium 下载到当前源码目录的 `browsers/`。
- 使用 PyInstaller `onedir` 模式打包。
- 生成 `dist/GeminiSearch/GeminiSearch.exe`。
- 在发布目录内创建 `profiles/default/` 和 `logs/`。
...
```text
dist/GeminiSearch/GeminiSearch.exe
```

运行后会：

- 使用当前 exe 所在文件夹作为运行目录。
- 使用 `browsers/` 作为 Playwright Chromium 目录。
- 使用 `profiles/default/` 作为默认浏览器 Profile。
- 使用 `logs/` 作为日志目录预留。
- 默认启动本地服务 `127.0.0.1:8080`，如果端口被占用会自动尝试后续端口。
- 自动打开测试控制台页面。
...
| 起始端口 | `8080` |
| 浏览器后端 | `playwright` |
| 浏览器模式 | 有头模式，`HEADLESS=0` |
| 浏览器目录 | 当前文件夹 `browsers/` |
| Profile 目录 | 当前文件夹 `profiles/default/` |

如果 Windows 禁止脚本运行，可以在 PowerShell 中临时允许当前会话执行脚本：
